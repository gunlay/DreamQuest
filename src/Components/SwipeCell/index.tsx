...
import { SwipeCellProps, ISwiperCellInstance } from './SwipeCell.d';
import './index.scss';
...
function Index(props: SwipeCellProps, ref: React.ForwardedRef<ISwiperCellInstance>) {
  ...
  const swipeMove = useCallback(
    function (offset2 = 0, dragging?: boolean) {
      ...
      const transition = dragging ? 'none' : 'transform .6s cubic-bezier(0.18, 0.89, 0.32, 1)';
      const wrapperStyle_ = {
        ...wrapperStyle,
  ...
  const open = useCallback(
    function (position) {
      const newOffset = position === 'left' ? leftWidth : -rightWidth;
      const THRESHOLD_ = {
        left: 0.2,
        right: 0.2,
      };
  ...
  const onDrag = useCallback(
    function (event) {
      if (disabled) return;
      const newTouchState = touchMove(event);
      if (!newTouchState.direction || newTouchState.direction === 'vertical') {
        return;
      }
      ARRAY.filter((item) => item.key !== instanceKey.key).forEach((item) => item.close(true));
      swipeMove(startOffset + newTouchState.deltaX, true);
    },
